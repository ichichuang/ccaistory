...
## 1. 任务定位
当 prompt、Lint 或依赖状态变化后，重新生成试产任务清单并保持依赖顺序可执行。
...
## 2. 前置状态要求
- 已有试产任务清单或 prompt/Lint 变更记录。
- 不得在未重新 Lint 的情况下把修改后 prompt 放入清单。
...
## 6. 详细执行步骤
1. 定位发生变化的 asset_id。
2. 确认变更后的 prompt 已重新通过语义 Lint。
3. 重排受影响资产及其下游依赖。
4. 保留已 accepted 且依赖未变化的资产状态。
5. 输出清单版本和变更摘要。

## 7. 质量门禁
- 不得覆盖执行遥测。
- 不得把 rejected 资产标成 accepted。
- 依赖不满足时清单项必须 blocked。
