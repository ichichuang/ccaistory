...
| output_assets | list | reference_asset id | [] | 输出资产 |
| failure_types | list | — | [] | 失败分类 |
| decision | select | accepted / rejected / repair | "" | 决策 |
| repair_package | text | repair_note id | "" | 关联返修 |
| human_approval | text | — | "" | 人工批准记录 |

## Validation notes / 校验说明

- `type` 必须为 `generation_run`。
- runtime 的 run/telemetry 产物是派生缓存；`decision=accepted` 必须回填到对应 reference_asset / execution package canonical 卡。
- `accepted` 前必须有 telemetry、image QA 与 asset QA，且有 `human_approval`。

## Dataview / Bases usage / 查询用法

- Dataview：`TABLE package_id, model, status, failure_types, created_at FROM "50-agent-work/story-lab/runs" WHERE type = "generation_run" SORT created_at DESC`
