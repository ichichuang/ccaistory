...
id: "出图执行技能"
title_zh: 出图执行技能
title_en: Image Execution Handoff Skill
status: active
project_id: ""
related_assets: []
source_paths: []
...
version: v1
canonical: true
skill_category: generation_handoff
runtime_commands: []
runtime_support_status: manual_only
input_layer: "02-wiki, 50-agent-work"
output_layer: "01-raw, 50-agent-work"
human_gate: yes
qa_gate: no
...
## 3. 输出
- 候选图路径、实际发送 prompt、执行事实
- machine_state 更新建议
- blocked_actions 或返修入口

## 4. 读取的唯一事实源字段
- pilot_task_list、compiled_prompt、reference_dependencies、r00_reference_asset、previous_page_reference、previous_page_scene_summary、current_page_scene_summary、scene_delta_from_previous_page、allowed_progression_delta、forbidden_continuity_breaks、page_hook_question、hook_visual_target、hook_annotation_guidance、escalation_level
...
- 输出字段能回写到 story_core.visual_pipeline.external_execution_queue。
...
- 人工执行单必须包含 Master Reference、Previous Page Continuity、Hook Direction、Escalation Control；缺任一段不得交给 WebGPTImage。

## 8. 失败回退位置
- 字段缺失：回退到故事核心状态机技能。
- 上游门禁失败：回退到 提示词巡检与任务清单技能。
...
- visual_pipeline.external_execution_queue
