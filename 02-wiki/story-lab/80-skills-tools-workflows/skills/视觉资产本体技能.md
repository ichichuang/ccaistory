...
id: "视觉资产本体技能"
title_zh: 视觉资产本体技能
title_en: Visual Asset Ontology Skill
status: active
project_id: ""
related_assets: []
source_paths: []
...
## 1. 技能目的
定义每个视觉资产的职责、内容边界、纸张策略、风格策略、依赖和验收问题。
...
## 4. 读取的唯一事实源字段
- visual_pipeline.asset_manifest、platform_story_pages、visual_rules
...
- 每个视觉资产规格必须包含非空 `paper_policy` 与 `style_policy`。
- 输出字段能回写到 story_core.visual_pipeline.asset_manifest、asset_ontology。
...
- visual_pipeline.asset_manifest、visual_pipeline.asset_ontology、machine_state

## 12. 必须检查的验收清单
- R00纸张笔触锚图验收清单、R01角色锚图验收清单、R02场景道具锚图验收清单、S页源插图验收清单
