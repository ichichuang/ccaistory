...
id: "平台图文故事生产流程"
title_zh: 平台图文故事生产流程
title_en: Platform Illustrated-Story Production SOP (main-line)
status: active
project_id: ""
related_assets: []
source_paths: []
...
version: v1
canonical: true
workflow_category: main_sop
trigger: ""
input_layer: "01-raw, 02-wiki"
output_layer: "02-wiki, 50-agent-work, 90-archive"
required_cards: []
runtime_commands: ["status", "can-run", "check-graph", "compile-asset", "lint-asset", "lint-prompt", "validate-telemetry", "qa-asset"]
human_gates: yes
qa_gates: yes
stop_conditions: []
replacement_for: ""
...
## 1. 流程目标
从故事核心一路推进到发布前检查，强调平台图文主线的硬门禁。

`runtime/` 是自动化生产执行层，负责把本文档规范转为本地可执行检查；本文档和 Obsidian 其他维护文件仍是规范层。

## 2. 输入
- 故事核心.json、平台目标、story_graph、视觉链路派生视图。
- 总控/状态机与门禁.md
- 对应 Codex 指令、生产技能、结构规范、模板和验收清单
- runtime CLI 检查结果：状态机、技法编排、graph check、Prompt compile、semantic lint、execution telemetry、asset QA。
...
- 禁止绕过 runtime 的 Prompt 编译、语义 Lint、执行遥测和资产 QA。
...
## 10. 下游入口
- 默认下游入口：[[00-system/codex-instructions/启动新故事并创建故事核心]]
...
## 12. runtime 执行要求
- 新故事生产前运行 `python runtime/aistory.py status`，确认空状态或当前 machine_state。
- 状态迁移前运行 `python runtime/aistory.py can-run --project <project_path> --action <action>`。
- story_graph 进入视觉链路前运行 `python runtime/aistory.py check-graph --project <project_path>`。
- 外部出图前运行 `compile-asset` 与 `lint-asset` / `lint-prompt`。
- 图片 accepted 前运行 `validate-telemetry` 与 `qa-asset`。
