...
id: "视觉执行编译流程"
title_zh: 视觉执行编译流程
title_en: Visual Execution Compile Flow (deprecated clone)
status: deprecated
project_id: ""
related_assets: []
source_paths: []
...
stop_conditions: []
replacement_for: ""
deprecated_by: "F-Prompt-Compile-and-Semantic-Lint-Workflow"
---
# 视觉执行编译流程
...
## 1. 流程目标
规定视觉资产本体、Prompt 编译、语义 Lint 和任务清单之间的不可跳过链路。
...
## 3. 输出
- 通过 Lint 的 compiled_prompt 与试产任务清单。
...
- 视觉项目总清单：列出资产、依赖和作用域。
- 视觉资产本体：定义 allowed_content、forbidden_content、paper_policy、style_policy、reference_dependencies 和验收问题。
...
- 视觉资产规格缺 `paper_policy` 或 `style_policy`。
...
## 10. 下游入口
- 默认下游入口：[[00-system/codex-instructions/编译源插图Prompt]]
