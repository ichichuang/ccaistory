...
> Workflow D human gate accepted (2026-06-26). StoryProject `pilot-001` is the final gate source; frontmatter remains `status: draft` because the PromptRecipe schema does not define `accepted` or `active`. Reusable recipe only. This card does not generate a concrete image, does not contain a WebGPTImage execution prompt, does not bind any single `image_id`, and must not be pasted into an image model as an execution sheet.

## Workflow D Gate / 工作流 D 门禁

Workflow D human gate accepted. This PromptRecipe is accepted for `pilot-001` through the StoryProject gate record, while this card retains schema-compliant `status: draft`. Stored `recipe_hash` is verified as `6020fc5e5c83e043`.
...
- Subject component: `{{approved_characters_from_character_cards}}` with locked clothing, proportions, and expressions.
