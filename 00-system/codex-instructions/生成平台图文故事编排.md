...
## 1. 任务定位
生成平台图文故事页面编排，明确每页图、文、钩子和阅读节奏。

成功时写入 `machine_state.current_state=platform_story_layout_generated`，并把 `next_allowed_action` 设为 `run_image_reading_test`。本指令是可执行 SOP，不依赖“按某流程执行”这类外包描述。

## 2. 前置状态要求
- `platform_story_graph_expanded` 或 `story_graph_hook_strengthened` 已通过。
- 不得跳过图读测试直接进入视觉资产本体。
...
- 故事核心.json.platform_story_pages
...
## 6. 详细执行步骤
1. 为每页写入 page_id、叙事目的、画面焦点、文字层、儿童手写线索和翻页钩子。
2. 标记每页是否需要源插图、锚图或平台页样张。
3. 检查页数、节奏和钩子是否与上游审查一致。
4. 把编排路径登记到 story_core.derived_views。
5. 通过后只解锁图读测试。
...
- 成功：写入 `current_state=platform_story_layout_generated`、`gate_result=passed`、`next_allowed_action=run_image_reading_test`。
...
- 下游入口只开放 `run_image_reading_test`。

## 12. 返回报告格式
- 是否完成：是/否
- 当前状态：platform_story_layout_generated
- 下一允许动作：run_image_reading_test
- 已读取文件：列出实际读取路径
- 已创建或更新文件：列出实际路径
